...
class BaseService():
    ...
    def _autoregister_handlers(self, bus: EventBus):
        ...
        for name, method in inspect.getmembers(self, predicate=inspect.isroutine):
            if getattr(method, '_autoregister', False):
                ...
                if event_type is None:
                    event_type = next(iter(sig.values())).annotation
                logger.debug(f"Autoregistering handler '{self.__class__.__name__}.{name}' for event type: {event_type}")
...
class ThreadService(BaseService):
    ...
    def stop(self):
        ...
        self._thread.join(0.5)  # Wait briefly for the thread to stop
        self._thread = None
